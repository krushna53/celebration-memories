...
/**
 * Marketing page for the Celebration Memories platform itself — distinct
 * from any individual event's page. This is where someone who wants to
 * build their own event site lands, not a guest of an existing event.
 */
interface PlatformMarketingPageProps {
  searchParams: Promise<{ ref?: string }>;
}

export default async function PlatformMarketingPage({ searchParams }: PlatformMarketingPageProps) {
  const { ref } = await searchParams;
  if (ref) {
    // Fire-and-forget-style await — small, non-blocking-in-practice write;
    // simplest correct approach without adding tracking middleware for
    // every public route just for this one query param.
        ...
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <Reveal>
            ...
            <p className="mx-auto mt-6 max-w-xl text-base leading-relaxed text-ivory-100/75 sm:text-lg">
              ...
              baby showers, and more. Unique guest links, live RSVP tracking,
              and a shared wall of photos, videos, and messages from everyone
              who came.
            </p>
