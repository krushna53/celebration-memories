 ...
 * /pay, /forms/build) are disallowed rather than left to a per-page
 * `noindex` meta tag on purpose — a crawler has to actually fetch a
 * page to see a noindex tag, and these URLs carry a bearer token in the
 * path itself (see the as-built CLAUDE.md's "possession of a token is
 * the credential" model). Disallowing the whole prefix means a
 * compliant crawler never requests the URL at all, so a token never
 * ends up in anyone's crawl logs. /business/dashboard and
 * /forms/dashboard are signed-in-only pages (nothing to index, and a
 * crawler can't reach past the login wall regardless) — note a
 * published Custom Form's own public fill page (/f/[slug]) is
 * deliberately NOT disallowed, same as any other public user-generated
 * page like /events/[slug]. /pricing-legacy is superseded by /pricing
 * and kept around only for old links — excluded so it doesn't compete
 * with the canonical page for search ranking.
 *
