...
/** Every form this account owns — the landing page after signing in at /forms/login or right after creating an account from the builder. */
export default async function FormsDashboardPage() {
  const owner = await getCurrentFormOwner();
  if (!owner) redirect("/forms/login");

  const forms = await listFormsForOwner(owner.id);
    ...
    <div className="min-h-screen bg-ivory-100">
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        <div className="flex items-center justify-between">
          <p className="text-sm text-navy-700/60">{forms.length} form{forms.length === 1 ? "" : "s"}</p>
          <Link
            href="/forms/new"
          ...
          {forms.map((form) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-navy-700/50 capitalize">{form.status}</p>
              </div>
            </Link>
          ))}
          {forms.length === 0 ? (
