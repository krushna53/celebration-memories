...
// re-uploads it into our own Storage (gallery bucket, so it's
// immediately compatible with the existing "Use as Link Preview Video"
// flow — see confirmShareVideoUploadAction), then marks the job row
// done and records a completed generation for the per-event quota.
//
// See generate-slideshow-video/index.ts for the "start" half of this
// flow and the README's "Slideshow Video" section for the full design.
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !shotstackKey) {
    console.error("slideshow-video-status: missing required environment variables");
  ...
  if (lookupError) {
    console.error(`slideshow-video-status: lookup failed for job ${jobId}:`, lookupError.message);
  ...
  try {
    ...
    console.error(`slideshow-video-status: status check failed for job ${jobId}: ${message}`);
    return jsonResponse({ success: true, status: "processing" }, 200);
  }
  ...
  if (shotstackStatus !== "done" || !shotstackUrl) {
    // queued, fetching, rendering, saving, ... — still in progress.
    return jsonResponse({ success: true, status: "processing" }, 200);
  }

  // Done on Shotstack's side — pull the finished MP4 down and store our
  // own copy so the result doesn't depend on Shotstack's own retention.
  try {
    ...
    const path = `${job.event_id}/slideshow-video/${crypto.randomUUID()}.mp4`;
    ...
    await supabase.from("slideshow_video_generations").insert({
